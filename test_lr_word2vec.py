...
if __name__ == '__main__':
    ...
    model_w2v.write().overwrite().save('model/w2v.model')
    model_w2v = Word2VecModel.load('model/w2v.model')

    text_df_all = model_w2v.transform(text_df)
    text_df_all.show()
    text_df_all.printSchema()
    print(text_df_all.count())

    print("以上是数据预处理部分的全部")

    featurescreator = VectorAssembler(inputCols=['token_count','text_token_vec'],outputCol='features')
    text_df_all = featurescreator.transform(text_df_all)
    df_train, df_test = text_df_all.randomSplit([0.8, 0.2])
    #
    # #调参
    # logistic = LogisticRegression(labelCol='cat_class')
    # grid = ParamGridBuilder().addGrid(logistic.maxIter, [2, 10, 50]).addGrid(logistic.regParam,
    #                                                                          [0.01, 0.05, 0.3]).build()
    # evaluator = MulticlassClassificationEvaluator(predictionCol='prediction', labelCol='cat_class')
    # cv = CrossValidator(estimator=logistic, estimatorParamMaps=grid, evaluator=evaluator)
    ...
    # # 取出所有组的参数
    # print(sorted(results, key=lambda el: el[1], reverse=True))


    ##########LR([{'maxIter': 50}, {'regParam': 0.01}]
    logistic = LogisticRegression(featuresCol='features',labelCol='cat_class')
    predictions = logistic.fit(text_df_all).transform(text_df_all)
    evaluator = MulticlassClassificationEvaluator(predictionCol='prediction', labelCol='cat_class',metricName='f1')
    f1 = evaluator.evaluate(predictions)
    print('使用word2vec进行处理的f1为',f1)



    # text_df_all_array = np.array(text_df_all.select('text_token_vec').collect())
    # #text_df_all_array是所有特征
    # text_df_all_array =text_df_all_array.reshape(text_df_all.count(), -1)
    # data = pd.DataFrame(text_df_all_array)
    #
    # label = pd.Series(np.array(text_df_all.select('cat_class').collect()).reshape(-1))
    #
